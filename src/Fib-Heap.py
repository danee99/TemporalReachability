...
class PriorityQueue:
    ...
    def heapify(self, i):
        ...
        r = 2*i+1
        smallest = i
        if l < self.heap_size and a[l][0] < self.heap[smallest]:
            smallest = l
        if r <= self.heap_size and a[r][0] < self.heap[smallest]:
            smallest = r
        if smallest != i:
     ...
     def insert(self, key, val):
        self.size += 1
        self.heap_list[self.size] = key
        self.map[self.heap_list[self.size]] = self.size
        i = self.size
    ...
    def sift_up(self, i):
        while i // 2 > 0:
            if self.heap_list[i][0] < self.heap_list[i // 2][0]:
                self.heap_list[i][0], self.heap_list[i // 2][0] = self.heap_list[i // 2][0], self.heap_list[i][0]
            i = i // 2

    def insert(self, k, v):
        self.heap_list.append((k,v))
        self.current_size += 1
    ...
    def sift_down(self, i):
        while (i * 2) <= self.current_size:
            ...
            if self.heap_list[i][0] > self.heap_list[mc][0] :
                self.heap_list[i][0] , self.heap_list[mc][0]  = self.heap_list[mc][0] , self.heap_list[i][0]
            i = mc
    ...
    def delete_min(self):
        if len(self.heap_list) == 1:
            return 'Empty heap'
        root = self.heap_list[1][0]
        self.heap_list[1][0]  = self.heap_list[self.current_size]
        *self.heap_list, _ = self.heap_list
        self.current_size -= 1
        self.sift_down(1)
        return root

    def decrease_Key(i, k, v):
        a[i][.key] = k
        while (i > 1 and a[i].key > a[bi / 2c].key):
            swap(a[i] a[bi / 2c])
            i = bi / 2c
